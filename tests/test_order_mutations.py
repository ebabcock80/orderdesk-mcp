...
class TestMergeOrderChanges:
    ...
    def test_merge_null_removes_field(self):
        ...
        changes = {"customer_notes": None}  # Explicit null
    ...
    def test_merge_array_replacement(self):
        ...
        original = {"id": "123", "order_items": [{"id": "1", "name": "Product A"}]}
...
class TestUpdateOrderWithRetry:
    ...
    async def test_update_succeeds_first_try(self):
        ...
        with patch.object(
            client, "fetch_full_order", new_callable=AsyncMock
        ) as mock_fetch:
            with patch.object(
                client, "update_order", new_callable=AsyncMock
            ) as mock_update:
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(client, "_backoff_fixed", new_callable=AsyncMock):
                    # First fetch
                    mock_fetch.return_value = {"id": "123", "email": "old@example.com"}
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(client, "_backoff_fixed", new_callable=AsyncMock):
                    ...
                    mock_update.side_effect = OrderDeskError(
                        "Conflict", code="CONFLICT_ERROR"
                    )

                    with pytest.raises(ConflictError) as exc_info:
                        await client.update_order_with_retry(
                            "123", {"email": "test"}, max_retries=3
                        )
    ...
    async def test_create_order_success(self):
        ...
        with patch("mcp_server.routers.orders.require_auth") as mock_auth:
            with patch("mcp_server.routers.orders.get_tenant_key") as mock_key:
                ...
                with patch(
                    "mcp_server.routers.orders.StoreService"
                ) as MockStoreService:
                    mock_store_service = MockStoreService.return_value
                    mock_store = MagicMock()
                    mock_store.store_id = "12345"
                    mock_store_service.resolve_store = AsyncMock(
                    ...
                    with patch("mcp_server.routers.orders.cache_manager") as mock_cache:
                        ...
                        with patch(
                            "mcp_server.routers.orders.OrderDeskClient"
                        ) as MockClient:
                            mock_client_instance = AsyncMock()
                            mock_client_instance.create_order = AsyncMock(
                                return_value={
                                    "id": "123456",
                                    "email": "customer@example.com",
                                    "order_total": 29.99,
                ...
                with patch(
                    ...
                    with patch("mcp_server.routers.orders.cache_manager") as mock_cache:
                        ...
                        with patch(
                            ...
                            mock_client_instance = AsyncMock()
                            mock_client_instance.update_order_with_retry = AsyncMock(
                                return_value={
                                    "id": "123456",
...
class TestDeleteOrderMCP:
    ...
    async def test_delete_order_success(self):
        ...
        params = DeleteOrderParams(order_id="123456", store_identifier="production")
                ...
                with patch(
                    ...
                    with patch("mcp_server.routers.orders.cache_manager") as mock_cache:
                        ...
                        with patch(
                            ...
                            mock_client_instance = AsyncMock()
                            mock_client_instance.delete_order = AsyncMock(
